...
const HamburgerMenu = ({ allCategories, setShowHamburger, showHamburger }) => {
    ...
    return (
        <div className={`${showHamburger ? 'block' : 'hidden'} ml-4 fixed w-full h-screen bg-white -left-4 z-50`}>
            <p className={` text-[1.1rem] font-medium pr-4 mr-4 py-3 select-none border-b-2 pl-8 border-[#e5e7eb]`}>Hi, <Link onClick={() => setShowHamburger(!showHamburger)} to={!localStorage.getItem('username') && '/login'} className='font-medium underline cursor-pointer'>{localStorage.getItem('username') ? localStorage.getItem('username') : 'Sign-in or register'}</Link></p>
            ...
            <SubcategoryMenu showSubcategory={showSubcategory} setShowSubcategory={setShowSubcategory} category={category} />
        </div>
    )
}
